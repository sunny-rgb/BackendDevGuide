...
2. 下载镜像，从 Docker官方仓库 里面下载最新版本的 mysql 镜像
...
2. 创建并运行 docker容器。需要注意的是， `docker pull` 可以省略，当发现本地不存在这个镜像，会自动拉取。此外，不同的可选参数代表不同的含义。
...
6. 创建 docker容器，和 `docker run` 命令非常相似，但是区别是 `docker create` 只创建容器，但是不立即启动。
